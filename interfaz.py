...
def juego():
    ...
    corazones = []
    for i in range(10):
        label_corazon = ctk.CTkLabel(frame_corazon, text="", image=imagen_corazon)
    ...
    frame_button_verificar = ctk.CTkFrame(ventana, fg_color="purple", width=1000, height=500)
...
def info(window):
    ...
    button_regresar = ctk.CTkButton(ventana, text="Regresar", font=("Arial", 20), width=10, height=2, command=lambda: return_to_menu(ventana))
...
def menu():
    ...
    imagen_fx_menu = ctk.CTkImage(light_image=Image.open("fx_menu.png").resize((400, 400)), size=(400, 400))
    ...
    button_jugar = ctk.CTkButton(frame_boton_jugar, text="Jugar", font=("Kristen ITC", 30), width=300, height=50, command=lambda: jugar(ventana))
    ...
    button_salir = ctk.CTkButton(frame_boton_salir, text="Salir", font=("Kristen ITC", 30), width=300, height=50, command=ventana.destroy)
