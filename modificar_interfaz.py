...
def verificar(respond,respond_entry,label_see,ask,label_ask,button,corazones,ventana,foto,aciertos):
    global i,vidas,puntos
    if respond==respond_entry.get(): # <--- Aquí se compara la respuesta del usuario con la respuesta correcta
        ...
        if i==len(ask) : # <--- Aquí se verifica si la lista de preguntas se acabó para reiniciarla
            ...
            return modificar_interfaz(preguntas,pregunta,respuestas,label_ask,button,respond_entry,label_see,corazones,ventana,foto,aciertos) # <--- Aquí se llama a la función que modifica la interfaz
...
def modificar_interfaz(ask,see,respond,label_ask,button,respond_entry,label_see,corazones,ventana,foto,aciertos): # <--- Aquí se crea la función que modifica la interfaz
    ...
    label_see.configure(text=ask[i]) # <--- Aquí se actualiza la pregunta
    button.configure(command=lambda: verificar(respond[i],respond_entry,label_see,ask,label_ask,button,corazones,ventana,foto,aciertos)) # <--- Aquí se le asigna la función verificar al botón